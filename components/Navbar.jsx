...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <div className="mx-auto max-w-7xl px-4 pt-4 md:px-8 md:pt-6">
        <nav className="flex h-[68px] items-center justify-between rounded-2xl border border-white/[0.08] bg-[#0b1424]/75 px-4 shadow-[0_8px_40px_rgba(0,0,0,0.25)] backdrop-blur-2xl md:px-6">
          {/* Logo */}
          <a href="/" className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center">
              <img
                src="/images/mlsc-su-logo.png"
                alt="MLSC × SU"
                className="h-full w-full object-contain drop-shadow-[0_0_14px_rgba(62,123,250,0.35)]"
              />
            </div>
          ...
          <div className="hidden items-center gap-1 md:flex">
            {links.map((link) => (
              ...
              >
                {link.name}
                <span className="absolute inset-x-3 -bottom-0.5 h-px origin-center scale-x-0 bg-gradient-to-r from-[#3E7BFA] to-[#FFB86B] transition-transform duration-300 group-hover:scale-x-100" />
              </a>
            ))}
          </div>
          ...
          {/* <a
            href="#membership"
            className="group hidden items-center gap-2 rounded-full bg-[#0F5FE0] px-5 py-2.5 text-[13px] font-semibold text-white shadow-[0_0_25px_rgba(62,123,250,0.25)] transition-all hover:bg-[#3E7BFA] hover:shadow-[0_0_35px_rgba(62,123,250,0.4)] md:flex"
          >
            Join MLSC
            <ArrowUpRight
              size={15}
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10, height: 0 }}
              transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}
              className="mt-2 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0b1424]/95 p-3 backdrop-blur-2xl md:hidden"
            >
              {links.map((link) => (
                <a
                  key={link.name}
              ...
              {/* <a
                href="#membership"
                onClick={() => setOpen(false)}
                className="mt-2 flex items-center justify-center gap-2 rounded-xl bg-[#0F5FE0] px-4 py-3 text-center text-sm font-semibold text-white"
              >
                Join MLSC
                <ArrowUpRight size={15} />
              </a> */}
