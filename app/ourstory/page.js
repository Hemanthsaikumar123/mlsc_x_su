...
import { ArrowLeft, ArrowDown, Sparkles, ArrowUpRight } from "lucide-react";
...
export default function OurStoryPage() {
  ...
  <Link
    href="/"
    className="mb-12 inline-flex items-center gap-2 text-xs text-white/85 transition hover:text-white"
  >
    <ArrowLeft size={14} />
    Back home
  </Link>
  ...
  >
    <div className="mb-6 flex items-center gap-3">
      <Sparkles size={15} className="text-[#50BFFF]" />
    ...
    <p className="mt-7 max-w-2xl text-base leading-8 text-white/75 md:text-lg">
      Every community has a beginning. Discover how MLSC × SU
      has grown through learning, collaboration and the students
        ...
        <div className="mx-auto max-w-5xl px-5 py-24 md:px-8 md:py-32">
          <div className="relative">
            ...
            {story.map((item, index) => (
              <StoryItem key={`${item.year}-${index}`} item={item} index={index} />
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="grid gap-12 md:grid-cols-[0.7fr_1.3fr] md:gap-20">
          ...
          <div>
            <h2 className="text-3xl font-semibold tracking-tight md:text-5xl">
              The story is still
              <span className="text-[#50BFFF]"> being written.</span>
            </h2>
